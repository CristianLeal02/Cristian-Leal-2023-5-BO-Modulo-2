...
class Enemy(Sprite):
    ...
    def __init__(self, rect_x):
        ...
        self.rect_x = rect_x
        self.select_type_enemy(self.type_enemy)
        self.enemy_speed_1 = random.randint(3, 6)
        self.enemy_speed_2 = random.randint(5, 9)
        self.count_updates = 0
        self.bullets = []
        self.count_updates_2 = 0
    ...
    def select_type_enemy(self, type_enemy): # selecciona el tipo de enemigo, sea 1 o 2 segun la variable
        if type_enemy <= 2:
    ...
    def select_move(self, type_enemy, spaceship): # selecciona el tipo de movimiento segun el tipo de enemigo
        if type_enemy <= 2:
            self.rect.y += self.enemy_speed_1
            self.move_zigzag()
            self.rect.x
        else:
            self.rect.y += self.enemy_speed_2
            self.move_chasing(spaceship)

    def move_zigzag(self): # movimiento del enemigo 1
        self.count_updates += 1
        if self.count_updates <= 30:
            self.rect.x -= self.enemy_speed_2
        elif self.count_updates < 60:
            self.rect.x += self.enemy_speed_2
        else:
            self.rect.x += self.enemy_speed_2
            self.count_updates = 0
    ...
    def move_chasing(self, spaceship): # movimiento del enemigo 2
        if spaceship > self.rect.x:
            self.rect.x += self.enemy_speed_1
        else:
            self.rect.x -= self.enemy_speed_1
